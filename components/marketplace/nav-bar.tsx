...
export function NavBar() {
  ...
  const MOCK_SUGGESTIONS = [
    { id: 'p-1', type: 'product', title: 'Wireless Headphones', subtitle: 'Electronics', href: '/product/p-1', image: '/modern-smartwatch.png' },
    { id: 'p-2', type: 'product', title: 'Stylish Leather Handbag', subtitle: 'Fashion', href: '/product/p-2', image: '/stylish-leather-handbag.png' },
    { id: 'p-3', type: 'product', title: 'Modern Smartwatch', subtitle: 'Electronics', href: '/product/p-3', image: '/modern-smartwatch.png' },
    { id: 'v-1', type: 'vendor', title: 'Green Grocers', subtitle: 'Groceries', href: '/vendor/v-1' },
    { id: 'v-2', type: 'vendor', title: "Gifty's Boutique", subtitle: 'Fashion', href: '/vendor/v-2' },
    { id: 'p-4', type: 'product', title: 'Stylish Leather Belt', subtitle: 'Fashion', href: '/product/p-4', image: '/assorted-vegetables.png' },
  ]
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-4">
              <span className="text-foreground">
                Call us: <span className="font-medium">+234 809 879 3038</span>
              </span>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="font-semibold text-lg hidden sm:inline">dispa8ch</span>
            </Link>
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  <div className="sm:hidden absolute left-2 top-1/2 -translate-y-1/2">
                    <Button variant="ghost" size="icon" onClick={() => setSearchModalOpen(true)} aria-label="Open search">
                  ...
                  <div className="hidden sm:block">
                    ...
                    {showSuggestions && filteredSuggestions.length > 0 && (
                      <div ref={suggestionsRef} role="listbox" aria-label="Search suggestions" className="absolute left-0 right-0 mt-1 bg-card border rounded shadow z-50 max-h-64 overflow-auto">
                        {filteredSuggestions.map((s, idx) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 hover:bg-accent/50 flex items-center gap-3 ${idx === highlightIndex ? 'bg-accent/60' : ''}`}
                          >
                            <div className="flex items-center gap-3">
                              {s.image ? (
                                ...
                                <div className="h-10 w-10 shrink-0 rounded bg-muted flex items-center justify-center text-sm text-muted-foreground">{s.type === 'vendor' ? 'V' : 'P'}</div>
                              )}
                              <div className="flex-1">
                                <div className="font-medium text-sm">{s.title}</div>
                                <div className="text-xs text-muted-foreground">{s.subtitle} • {s.type}</div>
                              </div>
                            </div>
                          </button>
                        ))}
                  ...
                  <div className="sm:hidden">
                    <Input aria-hidden placeholder="Search..." className="pl-10 pr-8" readOnly />
                  </div>
                </form>
              </div>
            </div>
              ...
              <div className="hidden sm:block">
                <Button variant="ghost" size="icon" className="relative" asChild>
                  <Link href="/wishlist">
                    ...
                    {likesCount > 0 && (
                      <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">{likesCount}</Badge>
                    )}
                  </Link>
                </Button>
                ...
                <Button variant="ghost" size="icon" className="relative" asChild>
                  <Link href="/cart">
                    ...
                    {cartCount > 0 && (
                      <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">{cartCount}</Badge>
                    )}
                  </Link>
                </Button>
              </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-2">
                    ...
                    <div className="sm:hidden">
                      <div className="flex items-center gap-2">
                        {likesCount > 0 && <Badge className="h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">{likesCount}</Badge>}
                        {cartCount > 0 && <Badge className="h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">{cartCount}</Badge>}
                      </div>
                    </div>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-64">
                  ...
                  <DropdownMenuItem asChild className="lg:hidden">
                    <Link href="/cart" className="flex items-center gap-2 group">
                      ...
                      <span>Cart</span>
                      <span className="ml-auto text-sm text-muted-foreground">{cartCount}</span>
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator className="lg:hidden" />
                  <DropdownMenuItem asChild>
                    <Link href="/settings/wallet" className="flex items-center gap-2 cursor-pointer group">
                      <Wallet className="h-4 w-4 group-hover:text-foreground" />
                      <span>Wallet</span>
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link href="/settings/orders" className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setIsLocationOpen(true); }} className="flex items-center gap-2 cursor-pointer">
                    <MapPin className="h-4 w-4" />
                    <span>Location</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="text-destructive flex items-center gap-2 cursor-pointer" onClick={() => setLogoutOpen(true)}>
      ...
      <SearchModal open={searchModalOpen} onOpenChange={setSearchModalOpen} initialQuery={searchTerm} initialCategory={selectedCategory} />
    </>
  );
}
