...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/auth/reset-page?email=${encodeURIComponent(email)}`);
    } catch (error: any) {
      toast({
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div className="hidden lg:flex flex-col relative bg-[#E41F47] text-white p-12 overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-30">
          <Image src="/placeholder.svg" alt="Abstract lines" fill className="object-cover" priority />
        </div>

        <div className="relative z-10 flex flex-col h-full justify-between">
          <div>
            <h1 className="text-[60px] leading-[0.9] mb-4">Recover Access</h1>
          </div>
